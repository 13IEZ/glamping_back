...
const fixApply = db => {
  db.once('open', async () => {
    ...
    const [geokupol, polimer, jetysu, tentland] = await Factory.create(
      { title: 'Компания «Геокупол»' },
      { title: 'Компания «Полимер»' },
      { title: 'СПК «Жетысу»' },
      { title: 'ТОО «TentLand»' }
    );
    ...
    const [orbita, ecohause, borovoelux, beskaynarlux] = await Pich.create(
      ...
      },
      {
        ...
        free: true,
        userId: testUser._id,
        description: 'Проведите неповторимые выходные в самом лоне природы!',
        published: true,
      },
      {
        title: 'Боровое Люкс',
      ...
      },
      {
        ...
        free: true,
        userId: testUser._id,
        description: 'Звезды,горы и посиделки у костра!',
        published: true,
      }
    );
